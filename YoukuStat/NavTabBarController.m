...
//初始化，用String
- (id) initWithtabName:(NSArray *)tabNames{
    self = [super init];
    if (self) {
        [self _commonInitWithString];
    }
    return self;
}
...
- (void)_commonInitWithString {
    self.segmentedControl = [[UISegmentedControl alloc]initWithItems:self.tabNames];
    self.segmentedControl.segmentedControlStyle = UISegmentedControlStyleBar;
    self.segmentedControl.momentary = NO;
    [self.segmentedControl addTarget:self action:@selector(_segmentTabNameChanged:) forControlEvents:UIControlEventEditingChanged];
}
...
- (void)_segmentChanged:(id)segmentChanged {
   ...
   [self setSelectedViewControllerIndex:(NSUInteger) control.selectedSegmentIndex animated:YES];
}
...
- (void) _segmentTabNameChanged:(id)segmentChanged{
    ...
    NSLog(@"NavTabBarController _segmentTabNameChanged:%d", control.selectedSegmentIndex);
}
...
- (void)viewDidLoad {
    NSLog(@"NavTabBarController ViewDidLoad");
    [super viewDidLoad];
